...
function buildServer() {
  ...
  server.registerTool(
    ...
    async ({ role, instructions, context }) => {
      const task = await store.createTask({ role, instructions, context });
  ...
  server.registerTool(
    ...
    async ({ worker_id, role }) => {
      const task = await store.claimTask({ worker_id, role });
      return { content: [{ type: "text", text: JSON.stringify(task, null, 2) }] };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ task_id, output }) => {
      const task = await store.submitResult({ task_id, output });
      return { content: [{ type: "text", text: JSON.stringify(task, null, 2) }] };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ task_id, verdict, notes }) => {
      const task = await store.reviewResult({ task_id, verdict, notes });
      return { content: [{ type: "text", text: JSON.stringify(task, null, 2) }] };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ status, role }) => {
      const tasks = await store.listTasks({ status, role });
  ...
  server.registerTool(
    ...
    async ({ worker_id, priority, content, data }) => {
      const signal = await store.fireSignal({ worker_id, priority, content, data });
  ...
  server.registerTool(
    ...
    async ({ priority }) => {
      const signals = await store.drainSignals({ priority });
